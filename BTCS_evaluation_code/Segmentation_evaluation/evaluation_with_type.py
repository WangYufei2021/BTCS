...
DIR = "/data/slurm/xuzt/ICA_data/BreaKHis_400X/test/"
DIR = Path(DIR)
# get list of files to run cellpose on

images = list(DIR.glob("**/*.png"))
images_list = [str(img) for img in images]
label = json.load(open('/data/slurm/xuzt/ICA_data/label.json', 'r'))
...
def get_predicted(img_path, User_given):
    User_given = label[img_path.split('/')[-1]]
...
with open('/data/slurm/xuzt/ICA_data/evaluation_results_with_type_test_only.json', 'w') as f:
    json.dump(results, f)
