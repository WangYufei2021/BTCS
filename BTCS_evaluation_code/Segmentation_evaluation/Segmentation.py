...
def parameter_defining(User_given="median"):
    """
    @brief:
...
def preprocess_image(image):
    ...
    # After attemps, we find select the first channel is good for nuclei segementation
...
def labeling_picture(processed_image,segmented):
    """
    @brief:
        Make segemented pictures into binary labeling
    @args:
        processed_image: The image that has only undergone preprocessing
        segemented: The image that has undergone preprocessing and K-means segementation
    @returns:
        Binary segemented labeled image.
    """
...
def filter_regions(label_image,user_given):
    ...
    @args:
        ...
        user_given: The image that has undergone preprocessing and K-means segementation
    @returns:
        Binary segemented labeled image.
    """
...
def remove_inner_objects(image):
    ...
    """
    @brief:
        Some nucleus have inner objects not detected (looks like concentric circles), so we want to fill them so that they will be segemented as a whole.
    @args:
        image: The input image
    @returns:
    ...
    # The largest one is the original background (but it can be somewhat segemented), so we can just set a otsu threshold to classify.
    thre_otsu=threshold_otsu(np.array(areas))
...
def Footprint_size(closed_image,level):
    ...
    """
    @brief:
        According to User given input estimation, we define a specific footprint size for subsequent instance segementation.
    @args:
...
def instance_segmentation(image,User_given):
    ...
    @returns:
        Labels after instance segementation using watershed segmentation
    """
    # If nuclus are very few, the instance segementation will not be processed, because we assume the nucleus don't overlap a lot.
    if User_given=='few': 
...
def nuclei_property(image,Measure_cells=False,Measure_regions=False,Calculate_length=False,Measure_eccentricity=False):
    ...
    @args:
        image: The segemented image without being edged.
    ...
    if Measure_regions:
        print(f"The mean area of nuclei is: {np.mean(areas)}")
        print(f"The median area of nuclei is: {np.median(areas)}")
...
def nuclei_segementation(image_path,User_given,Plot='one',save=False, save_path=None,Measure_cells=False,Measure_regions=False,Calculate_length=False,Measure_eccentricity=False):
    """
    @brief:
        Nuclei segementation undergoes preprocessing, segementation, binary labeling, closing, removing inner objects, instance segmentation, filter regions, and getting edges. 
    @args:
        image_path: The image path.
        User_given: User given input estimation. "Few": <10 cells; "median": 10~50 cells; "many": > 50 cells
        Plot: Do you want to plot only the final edged image, or the whole processing steps. 
    @returns:
    ...
    return second_removed_image
    # if save_path==None:
    #     # The file will be stored in where the original image stays with "_segmented" at the end.
    #     save_list=image_path.split(".")
    #     format=save_list[-1]
    #     save_list.pop()
    #     save_list.append("_segemented.")
    #     save_list.append(format)
    #     save_path=''.join(save_list)
    # save_path = '/Users/apple/Desktop/大四上/BIA4/ICA/code/test.png'

    # if Plot=="one":
    #     # plt.imshow(edged_image)
    #     fig = plt.figure(figsize=(4, 4))
    #     plt.imshow(second_removed_image, cmap='gray')
    #     if save:
    #         plt.axis('off')
    #         plt.savefig(save_path, dpi=500)
    #         print(f"The new image is saved in the path: {save_path}")
    #     plt.show()
    # else:
    ...
    #     ax[1,0].set_title('Image after removing small inner objects')
    #     ax[1,1].imshow(updated_regions,cmap='gray')
    #     ax[1,1].set_title('Image after filtering regions')
    #     ax[1,2].imshow(second_removed_image,cmap='gray')
    #     ax[1,2].set_title('Image after second removeing small innner objects')
...
image_path = '/Users/apple/Desktop/大四上/BIA4/ICA/data/BreaKHis_400X/train/benign/SOB_B_A-14-22549AB-400-002.png' # Replace with your image path
User_given='few'
Measure_cells=True
Measure_regions=True
Calculate_length=True
Measure_eccentricity=True

# nuclei_segementation(image_path,User_given,"all",save=True, Measure_cells=True,Measure_regions=True,Calculate_length=True,Measure_eccentricity=True)
# nuclei_segementation(image_path,User_given,"one",save=True, Measure_cells=True,Measure_regions=True,Calculate_length=True,Measure_eccentricity=True)
